...
train_seq_length = 4
model_name = 'LinearBracketCounter'
task='BracketCounting'
...
# model = LinearBracketCounter(input_size,hidden_size, counter_input_size, counter_output_size,output_size)
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 10 == 0:
        ...
        print('output layer weights  = ', model.out.weight)
        print('output layer weight gradient = ', model.out.weight.grad)
        print('output layer bias = ',model.out.bias)
    ...
    # weights_input_layer.append(model.fc1.weight.clone())
    weights_output_layer.append(model.out.weight.clone())
    # gradients_input_layer.append(model.fc1.weight.grad.clone())
    gradients_output_layer.append(model.out.weight.grad.clone())
    weights_counter.append(model.counter.weight.clone())
    gradients_counter.append(model.counter.weight.grad.clone())
    gradients.append(input_tensor.grad.clone())
    ...
    if epoch == num_epochs - 1:
        ...
        if i == len(X_train) - 1:
            ...
            print('output layer weight = ', model.out.weight)
            print('output layer weight gradient = ', model.out.weight.grad)
